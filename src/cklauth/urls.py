...
urlpatterns = [
    ...
    url(r'^auth/password/change/$', views.password_change, name='account_change_password'),
    ...
    url(r'^auth/confirm-email/$', views.email_verification_sent, name='account_email_verification_sent'),
    ...
    url(r'^auth/password/reset/done/$', views.password_reset_done, name='account_reset_password_done'),
    url(r'^auth/password/reset/key/(?P<uidb36>[0-9A-Za-z]+)-(?P<key>.+)/$',
        views.password_reset_from_key,
        name='account_reset_password_from_key'),
    url(r'^auth/password/reset/key/done/$', views.password_reset_from_key_done,
        name='account_reset_password_from_key_done'),
]

if 'allauth.socialaccount' in settings.INSTALLED_APPS:
    urlpatterns += [
        url('^auth/social/login/cancelled/$', social_views.login_cancelled, name='socialaccount_login_cancelled'),
        url('^auth/social/login/error/$', social_views.login_error, name='socialaccount_login_error'),
        url('^auth/social/signup/$', social_views.signup, name='socialaccount_signup'),
        url('^auth/social/connections/$', social_views.connections, name='socialaccount_connections'),
